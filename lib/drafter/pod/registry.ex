defmodule Drafter.Pod.Registry do
  ...
  @spec picks(Player.playerID(), State.channelID()) :: :ok
  ...
  # helpers
  # registry stuff
  @spec whereis_pod(State.pod_name(), state()) :: State.pod_pid() | :undefined
  defp whereis_pod(pod_name, %{pods: pods}) do
    Map.get(pods, pod_name, :undefined)
  end

  @spec whereis_player(Player.playerID(), state()) :: State.pod_name() | :undefined
  defp whereis_player(playerID, %{locations: locations}) do
    Map.get(locations, playerID, :undefined)
  end

  @spec register_name(State.pod_name(), Player.group(), State.pod_pid(), state()) :: state()
  ...
  @spec first_free([State.pod_name()], integer()) :: State.pod_name()
  ...
  # maintenance
  @spec handle_cast({:kill_pod, State.pod_string(), State.channelID()}, state()) ::
          {:noreply, state()}
  ...
  @spec handle_cast({:kill_all, State.channelID()}, state()) :: {:noreply, state()}
  ...
  @spec handle_cast({:prune, State.channelID()}, state()) :: {:noreply, state()}
    ...
    case whereis_pod(pod_atom, state) do
      :undefined ->
        if verify_group(group, state) do
          {:ok, pid} = State.start_link(pod_atom, {set, option, group})
  ...
  @spec handle_cast({:ready_player, Player.playerID(), State.channelID()}, state()) ::
          {:noreply, state()}
  def handle_cast({:ready_player, playerID, channelID}, state) do
    ...
    case whereis_pod(pod_name, state) do
      ...
      _pid ->
        State.ready(pod_name, playerID, channelID)
        {:noreply, pruned(state)}
    end
  end

  # running
  @spec handle_cast({:pick, Player.playerID(), String.t(), State.channelID()}, state()) ::
          {:noreply, state()}
    ...
    case Integer.parse(card_index_string) do
      ...
      {index, _} ->
        case whereis_player(playerID, state) do
          ...
          pod_name ->
            State.pick(pod_name, playerID, index)
            {:noreply, pruned(state)}
        end
    end
  end

  @spec handle_cast({:picks, Player.playerID(), State.channelID()}, state()) ::
          {:noreply, state()}
